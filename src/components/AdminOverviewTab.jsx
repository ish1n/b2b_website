...
import { FiArrowRight, FiChevronDown, FiFilter, FiX } from "react-icons/fi";
...
function filterOrdersForRow({ modalFilterProperty, modalFilterStatus, row, orders }) {
  return orders.filter((order) => {
    ...
    if (modalFilterProperty !== "All" && (order.property || order.tenant) !== modalFilterProperty) {
      return false;
    }
...
export default function AdminOverviewTab({ orders, daysInRange }) {
  ...
  const { categoryBreakdown, dailyRevenue, propertyRows } = useOverviewMetrics({ orders, daysInRange });

  const visibleRows = useMemo(() => {
    let rows = [...propertyRows];

    if (filterType !== "All") {
      rows = rows.filter((row) => row.hostelType === filterType);
    }
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-100 shadow-sm p-6 pb-2 min-w-0">
          ...
          <ResponsiveContainer width="100%" height={window.innerWidth < 640 ? 220 : 300} debounce={100} minWidth={1} minHeight={1}>
            <AreaChart data={dailyRevenue} margin={{ top: 10, right: 10, left: window.innerWidth < 640 ? -25 : -10, bottom: 0 }}>
              ...
              <XAxis dataKey="day" tick={{ fontSize: 10, fill: "#94A3B8", fontWeight: 500 }} axisLine={false} tickLine={false} dy={10} interval={window.innerWidth < 640 ? 2 : 0} />
              <YAxis tick={{ fontSize: 10, fill: "#94A3B8", fontWeight: 500 }} axisLine={false} tickLine={false} tickFormatter={(value) => `₹${(value / 1000).toFixed(0)}k`} width={window.innerWidth < 640 ? 35 : 50} />
              ...
              <div key={category.label} className="group">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center justify-end gap-0.5 text-[13px] font-extrabold text-[#0F172A]">
                    <BiRupee size={12} />
                    <span>{category.revenue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                  </div>
                </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-gray-50 flex flex-wrap items-center justify-between gap-4 bg-white">
          <div>
            ...
            <p className="text-[12px] font-medium text-[#94A3B8]">Metrics grouped directly from live order properties</p>
          </div>
              ...
              {showFilterMenu && (
                <div className="absolute right-0 mt-2 w-40 bg-white border border-gray-100 rounded-xl shadow-xl z-20 overflow-hidden py-1">
                  {TYPE_FILTERS.map((filter) => (
                    <button key={filter} onClick={() => { setFilterType(filter); setShowFilterMenu(false); }} className={`w-full text-left px-4 py-2 text-[12px] font-bold transition-colors ${filterType === filter ? "bg-blue-50 text-blue-600" : "text-slate-600 hover:bg-slate-50"}`}>
                      {filter} {filter === "All" ? "Types" : "Properties"}
                    </button>
                  ))}
                </div>
              )}
              ...
              {showSortMenu && (
                <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-xl z-20 overflow-hidden py-1">
                  {[
                    ...
                    <button key={`${sort.key}-${sort.dir}`} onClick={() => { setSortConfig({ key: sort.key, direction: sort.dir }); setShowSortMenu(false); }} className={`w-full text-left px-4 py-2 text-[12px] font-bold transition-colors ${sortConfig.key === sort.key && sortConfig.direction === sort.dir ? "bg-blue-50 text-blue-600" : "text-slate-600 hover:bg-slate-50"}`}>
                      {sort.label}
                    </button>
                  ))}
                </div>
          ...
          <table className="w-full min-w-[850px]">
            ...
            <tbody>
              {visibleRows.map((row, index) => (
                <tr key={row.id} className="border-b border-gray-50 hover:bg-[#F8FAFC] transition-colors group">
                  ...
                  <td className="px-6 py-4 text-[13.5px] font-black text-[#0F172A] text-right">
                    <div className="flex items-center justify-end gap-0.5">
                      <BiRupee size={14} />
                      <span>{row.rev.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-[13px] font-bold text-[#64748B] text-right">
                    {row.orders > 0 ? (
                      <div className="flex items-center justify-end gap-0.5">
                        <BiRupee size={12} />
                        <span>{(row.rev / row.orders).toFixed(0)}</span>
                      </div>
                    ) : "—"}
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => handleOpenPropertyModal(row)} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 text-[#475569] text-[12px] font-bold hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all">
        ...
        <div className="md:hidden divide-y divide-gray-50">
          {visibleRows.map((row, index) => (
            <div key={row.id} onClick={() => handleOpenPropertyModal(row)} className="p-4 active:bg-gray-50 transition-colors cursor-pointer">
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className={`text-[9px] font-black px-1.5 py-0.5 rounded uppercase tracking-wider ${getStatusBadgeClass(row.hostelType).replace("100", "50").replace("700", "600")}`}>{row.hostelType}</span>
                  </div>
                </div>
                <div className="text-right">
                  <div className="flex items-center justify-end gap-0.5 text-sm font-black text-[#0F172A]">
                    ...
                    <span>{row.rev.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-3 pt-3 border-t border-gray-100/50">
                <div className="bg-slate-50/50 p-2 rounded-lg border border-slate-100/50">
                  ...
                  <span className="text-[11px] font-black text-slate-700">{row.kg > 0 ? row.kg.toFixed(1) : "0"} KG</span>
                </div>
                <div className="bg-slate-50/50 p-2 rounded-lg border border-slate-100/50">
                  ...
                  <div className="flex items-center gap-0.5 text-[11px] font-black text-slate-700">
                    ...
                    <span>{(row.rev / (row.orders || 1)).toFixed(0)}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="relative bg-white sm:rounded-2xl shadow-2xl w-full h-full sm:h-auto sm:max-w-5xl sm:max-h-[90vh] flex flex-col overflow-hidden animate-slide-up sm:animate-fade-in" style={{ fontFamily: "DM Sans, sans-serif" }}>
            <div className="flex items-center justify-between p-6 border-b border-gray-100 flex-shrink-0">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${getStatusBadgeClass(selectedProperty.hostelType).replace("100", "50").replace("700", "600")}`}>{selectedProperty.hostelType}</span>
            ...
            <div className="bg-gray-50 p-4 border-b border-gray-100 grid grid-cols-2 sm:grid-cols-5 gap-3 sm:gap-4 flex-shrink-0">
              ...
              <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm text-center">
                ...
                <div className="flex items-center justify-center gap-0.5 text-lg font-bold text-[#1976D2]">
                  <BiRupee size={18} />
                  <span>{selectedProperty.rev.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                </div>
              </div>
              <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm text-center">
                ...
                <div className="flex items-center justify-center gap-0.5 text-lg font-bold text-gray-800">
                  <BiRupee size={16} />
                  <span>{selectedProperty.orders > 0 ? (selectedProperty.rev / selectedProperty.orders).toFixed(0) : "—"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-4 flex-shrink-0">
              <div className="flex items-center gap-2 overflow-x-auto pb-2 sm:pb-0 scrollbar-hide">
                ...
                <div className="flex gap-2">
                  {STATUS_FILTERS.map((status) => (
                    <button key={status} onClick={() => setModalFilterStatus(status)} className={`px-3 py-1.5 rounded-full text-[10px] sm:text-[11px] font-bold whitespace-nowrap transition-all ${modalFilterStatus === status ? "bg-slate-800 text-white shadow-md" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>
                      {status}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-2 border-t sm:border-t-0 pt-3 sm:pt-0">
                ...
                <select value={modalFilterProperty} onChange={(event) => setModalFilterProperty(event.target.value)} className="bg-slate-50 border border-slate-200 text-slate-700 text-[11px] sm:text-[12px] font-bold rounded-lg px-3 py-2 sm:py-1.5 outline-none focus:border-blue-500 min-w-[140px]">
