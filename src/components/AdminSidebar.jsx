...
export default function AdminSidebar({ activeTab, setActiveTab, issuesCount, user, onLogout, isCollapsed, setIsCollapsed, isMobileOpen, setIsMobileOpen }) {
    ...
    return (
        <>
            ...
            {/* Navigation */}
            <nav className={`flex-1 mt-4 space-y-1 ${isCollapsed ? 'px-3' : 'px-3'}`}>
                {menuItems.map((item) => (
                    <button
                        key={item.key}
                        onClick={() => setActiveTab(item.key)}
                        title={isCollapsed ? item.label : ''}
                        className={`w-full flex items-center rounded-lg text-[13.5px] font-medium transition-all group ${isCollapsed ? 'justify-center py-3.5' : 'justify-between px-4 py-3'} ${
                            activeTab === item.key
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <item.icon size={20} className={activeTab === item.key ? 'text-white' : 'text-[#475569] group-hover:text-blue-400'} />
